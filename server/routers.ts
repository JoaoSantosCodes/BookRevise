...
import { claimNextReviewJob, enqueueReview, finishReviewJob, getBookForUser, getDb, getReviewJobForBook, listBooks, listIssues, updateIssue, books, reviewIssues, bookVersions, reviewJobs } from "./db";
...
export async function processQueuedReview() { const job = await claimNextReviewJob(); if (!job) return { status: "empty" as const }; try { const issueCount = await analyzeBook(job.bookId); await finishReviewJob(job.id, "completed"); return { status: "completed" as const, issueCount, bookId: job.bookId }; } catch (error) { await finishReviewJob(job.id, "failed", error instanceof Error ? error.message : "Falha desconhecida"); return { status: "failed" as const, bookId: job.bookId }; } }
...
export const appRouter = router({
  ...
  books: router({
    ...
    compareVersions: protectedProcedure.input(z.object({ bookId: z.number(), fromVersionId: z.number().nullable(), toVersionId: z.number() })).query(async ({ ctx, input }) => { const book = await getBookForUser(input.bookId, ctx.user.id); if (!book) throw new Error("Livro não encontrado"); const versions = await listVersions(book.id); const from = input.fromVersionId ? versions.find(v => v.id === input.fromVersionId)?.contentText ?? "" : book.manuscriptText; const to = versions.find(v => v.id === input.toVersionId)?.contentText ?? ""; return { from, to, segments: diffText(from, to) }; }),
