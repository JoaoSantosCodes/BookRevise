...
import { claimNextReviewJob, createDiffAnnotation, deleteDiffAnnotation, enqueueReview, finishReviewJob, getBookForUser, getDb, getReviewJobForBook, getUserPreferences, listBooks, listDiffAnnotations, listIssues, listVersions, rescheduleReviewJob, updateIssue, updateUserPreferences, books, reviewIssues, bookVersions, reviewJobs } from "./db";
...
export async function processQueuedReview() { const job = await claimNextReviewJob(); if (!job) return { status: "empty" as const }; try { const issueCount = await analyzeBook(job.bookId); await finishReviewJob(job.id, "completed"); return { status: "completed" as const, issueCount, bookId: job.bookId }; } catch (error) { const detail = error instanceof Error ? error.message.replace(/[\r\n]+/g, " ").slice(0, 240) : "O serviço de análise retornou uma falha inesperada."; const message = `A análise não foi concluída: ${detail}`; const retry = await rescheduleReviewJob(job.id, job.attempts, message); return { status: retry.status, bookId: job.bookId, attempts: job.attempts, message }; } }
...
export const appRouter = router({
  ...
  auth: router({
    ...
    preferences: protectedProcedure.query(async ({ ctx }) => { const raw = await getUserPreferences(ctx.user.openId); if (!raw) return { shortcuts: { previous: "ArrowLeft", next: "ArrowRight", first: "Home", last: "End" }, darkMode: false }; try { return JSON.parse(raw); } catch { return { shortcuts: { previous: "ArrowLeft", next: "ArrowRight", first: "Home", last: "End" }, darkMode: false }; } }),
    savePreferences: protectedProcedure.input(z.object({ shortcuts: z.object({ previous: z.string().min(1).max(32), next: z.string().min(1).max(32), first: z.string().min(1).max(32), last: z.string().min(1).max(32) }), darkMode: z.boolean() })).mutation(async ({ ctx, input }) => { const values = Object.values(input.shortcuts); if (new Set(values).size !== values.length) throw new Error("Os atalhos precisam ser diferentes entre si."); await updateUserPreferences(ctx.user.openId, JSON.stringify(input)); return input; }),
  }),
  books: router({
    ...
    listDiffAnnotations: protectedProcedure.input(z.object({ bookId: z.number(), versionId: z.number() })).query(async ({ ctx, input }) => { const book = await getBookForUser(input.bookId, ctx.user.id); if (!book) throw new Error("Livro não encontrado"); return listDiffAnnotations(ctx.user.id, book.id, input.versionId); }),
    createDiffAnnotation: protectedProcedure.input(z.object({ bookId: z.number(), versionId: z.number(), kind: z.enum(["highlight", "comment"]), excerpt: z.string().trim().min(1).max(4000), note: z.string().trim().max(4000).optional(), color: z.string().regex(/^#[0-9a-fA-F]{6}$/) })).mutation(async ({ ctx, input }) => { const book = await getBookForUser(input.bookId, ctx.user.id); if (!book) throw new Error("Livro não encontrado"); const version = (await listVersions(book.id)).find(item => item.id === input.versionId); if (!version) throw new Error("Versão não encontrada"); if (input.kind === "comment" && !input.note) throw new Error("Comentários precisam de texto."); const id = await createDiffAnnotation({ userId: ctx.user.id, bookId: book.id, versionId: version.id, kind: input.kind, excerpt: input.excerpt, note: input.note ?? null, color: input.color }); return { id, ...input }; }),
    deleteDiffAnnotation: protectedProcedure.input(z.object({ bookId: z.number(), annotationId: z.number() })).mutation(async ({ ctx, input }) => { const book = await getBookForUser(input.bookId, ctx.user.id); if (!book) throw new Error("Livro não encontrado"); await deleteDiffAnnotation(ctx.user.id, input.annotationId); return { success: true }; }),
    ...
    exportDiffPdf: protectedProcedure.input(z.object({ bookId: z.number(), fromVersionId: z.number().nullable(), toVersionId: z.number() })).mutation(async ({ ctx, input }) => { const book = await getBookForUser(input.bookId, ctx.user.id); if (!book) throw new Error("Livro não encontrado"); const versions = await listVersions(book.id); const from = input.fromVersionId ? versions.find(v => v.id === input.fromVersionId)?.contentText ?? "" : book.manuscriptText; const to = versions.find(v => v.id === input.toVersionId)?.contentText ?? ""; if (!to) throw new Error("Versão de destino não encontrada"); const body = diffWords(from, to).map(segment => `${segment.type === "added" ? "+ " : segment.type === "removed" ? "- " : "  "}${segment.text}`).join(""); const pdfBuffer = await makePdf(`${book.title} — Diff`, `Comparação palavra por palavra\n\n${body}`); const uploaded = await storagePut(`${ctx.user.id}/versions/${book.title}-diff.pdf`, pdfBuffer, "application/pdf"); const db = await getDb(); if (!db) throw new Error("Banco indisponível"); const versionNumber = (versions[0]?.versionNumber ?? 0) + 1; await db.insert(bookVersions).values({ bookId: book.id, kind: "pdf", filename: `${book.title}-diff.pdf`, fileKey: uploaded.key, fileUrl: uploaded.url, contentText: body, versionNumber, baseVersionId: input.toVersionId }); return uploaded;
    }),
